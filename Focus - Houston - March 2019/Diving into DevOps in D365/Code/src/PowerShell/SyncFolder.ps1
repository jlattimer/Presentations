# https://herringsfishbait.com/2015/01/23/powershell-synchronizing-a-folder/
...
.PARAMETER TargetFolders
    The full path to the target folder(s) that the source should be synched to.
.PARAMETER PassedExceptions
    An array of all the exceptions passed to be checked.
.PARAMETER Filter
    ...
    Foreach ($TargetFolder in $TargetFolders) {
        ...
        ForEach ($TargetFile in $FilteredTargetFiles) {
            ...
            If ($MatchingSourceFile -ne $Null) {
                $hash1 = Get-FileHash -Path $MatchingSourceFile.FullName
                $hash2 = Get-FileHash -Path $TargetFile.FullName
                If (Compare-Object -ReferenceObject $hash1.Hash  -DifferenceObject $hash2.Hash) {
                    $Output = New-ReportObject
